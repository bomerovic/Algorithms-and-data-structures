...
unsigned int ney(int s, unsigned int max) {
    return s+s;
}
...
class HashMapa : public Mapa<tip1,tip2> {
    ...
    int kapacitet=100;
    int br_el=0;
    unsigned int (*h)(tip1, unsigned int)=0;
    public:
    void definisiHashFunkciju(unsigned int (*fun)(tip1, unsigned int)) {h=fun;}
    HashMapa() {
        niz = new Pair<tip1,tip2>[kapacitet];
    }
    HashMapa(const HashMapa<tip1,tip2> &m) : kapacitet(m.kapacitet), br_el(m.br_el), h(m.h) {
        niz = new Pair<tip1,tip2>[m.kapacitet];
        for(int i(0); i<m.kapacitet; i++)
            niz[i] = m.niz[i];
    }
    HashMapa(HashMapa<tip1,tip2> &&m) : kapacitet(m.kapacitet), br_el(m.br_el), h(m.h) {
        niz=m.niz;
    }
    HashMapa &operator = (const HashMapa<tip1,tip2> &m) {
        if(this==&m) return *this;
        delete[] niz;
        kapacitet = m.kapacitet;
        br_el = m.br_el;
    ...
    HashMapa &operator = (HashMapa<tip1,tip2> &&m) {
        if(this==&m) return *this;
        delete[] niz;
        kapacitet=m.kapacitet;
        br_el=m.br_el;
    ...
    tip2 &operator[](const tip1 &a) override {
        ...
        int n=h(a,kapacitet);
        while(n<kapacitet && niz[n].kljuc!=tip1()) n++;
        if(n>=kapacitet) {
            Pair<tip1,tip2>* novi=new Pair<tip1,tip2>[kapacitet+50];
            for(int i(0); i<kapacitet; i++) {
                novi[i]=niz[i];
            }
            kapacitet+=50;
        ...
        br_el++;
        niz[n].kljuc=a;
        niz[n].vrijednost=tip2();
        return niz[n].vrijednost;
    }
    void obrisi() {
        delete [] niz;
        kapacitet=100; br_el=0;
...
void Test() {
    ...
    generisi(n,4000);
    clock_t v3=clock();
    h.definisiHashFunkciju(ney);
    generisi(h,4000);
    clock_t v4=clock();
    double uk1=(v2-v1)/(CLOCKS_PER_SEC/1000);
    double uk2=(v3-v2)/(CLOCKS_PER_SEC/1000);
    double uk3=(v4-v3)/(CLOCKS_PER_SEC/1000);
    std::cout<<"Binarno stablo "<<uk1/4000 << std::endl << "Niz mapa " << uk2/4000 << std::endl << "Hash mapa " << uk3/4000 << std::endl;
    ...
    clock_t v7=clock();
    for(int i(0); i<4000; i++) h[i];
    clock_t v8=clock();
    double uk4=(v6-v5)/(CLOCKS_PER_SEC/1000);
    double uk5=(v7-v6)/(CLOCKS_PER_SEC/1000);
    double uk6=(v8-v7)/(CLOCKS_PER_SEC/1000);
    std::cout<<"Binarno stablo "<<uk4/4000 << std::endl << "Niz mapa " << uk5/4000 << std::endl<< "Hash mapa " << uk6/4000 << std::endl;
}
